...
const baileyCleanNumber$1 = (number, full = false) => {
    if (number.endsWith('@g.us') || number.endsWith('@s.whatsapp.net')) {
        return number; // Si ya es válido, retornarlo
    }
   ...
   busEvents = () => [
        {
            ...
            func: ({ messages, type }) => {
                ...
                if (messageCtx.message?.videoMessage || messageCtx.message?.stickerMessage || messageCtx.message?.imageMessage || messageCtx.message?.documentMessage || messageCtx.message?.audioMessage) {
                    payload = { ...payload, body: generateRefprovider('_event_media_') };
                }
                // Procesar tarjetas de contacto
            if (messageCtx.message?.contactMessage || messageCtx.message?.contactsArray) {
                const contacts = [];

                // Procesar una sola tarjeta de contacto
                if (messageCtx.message.contactMessage) {
                    const vcard = messageCtx.message.contactMessage.vcard;

                    if (vcard) {
                        const matches = [...vcard.matchAll(/waid=(\d+)/g)];
                ...
                // Procesar múltiples tarjetas de contacto
                if (messageCtx.message.contactsArrayMessage) {
                    for (const contactObj of messageCtx.message.contactsArrayMessage.contacts) {
                        if (contactObj?.vcard) {
                ...
                if (contacts.length > 0) {
                    payload = { ...payload, body: contacts.join(' ') };
                } else {
                    console.log("Error: No se encontró ningún waid válido.");
                }
